...
loginRouter.post('/', userUpdateParser, async (req: Request<unknown, unknown, newUsernameEntry>, res:Response) => {
    ...
    const passwordCorrect = userToAuthenticate === null ? 
        false : 
        await bcrypt.compare(password,(userToAuthenticate.get({ plain: true}) as newUserEntry).password)
    ...
    //If the credentials are valid, the code creates a token payload (userForToken) containing the username
    const userForToken = {
        username: username
    }
